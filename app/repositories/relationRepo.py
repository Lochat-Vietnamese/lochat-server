...
class RelationRepo:
    ...
    def all(page: int, page_size: int, is_active: bool | None):
        try:
            if is_active is None:
                queryset = Relation.objects.select_related("first_user", "second_user").all().order_by("-created_at")
            else:
                queryset = Relation.objects.select_related("first_user", "second_user").filter(
                    is_active=is_active
                ).order_by("-created_at")
    ...
    def find_by_id(relation_id: uuid.UUID, is_active: bool | None):
        try:
            if is_active is None:
                return Relation.objects.select_related("first_user", "second_user").get(id=relation_id)
            else:
