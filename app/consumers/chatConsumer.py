...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect_room(self, conversation, *profiles):
        ...
        self._current_sender_relation = await sync_to_async(
            ProfileConversationService.get_by_both
        )(conversation_id=self._room_id, profile_id=self._current_user.id)
                ...
                json.dumps(
                    {
                        "message": (
                            ...
                            + f"Đã kết nối với {profile.nickname}"
                            if profile != self.scope["user"]
                            else "Đã kết nối"
                        )
                    },
                    ensure_ascii=False,
        ...
        try:
            ...
            if isinstance(self._current_user, AnonymousUser):
                ...
                represent_conversation = await sync_to_async(
                    ConversationService.find_by_id
                )(id)
                if not represent_conversation:
                    friend = await sync_to_async(AccountService.find_by_id)(id)
                    if friend:
                        common_conversation = await sync_to_async(
                            ProfileConversationService.find_common_conversation
